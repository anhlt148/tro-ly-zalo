...
## Bước 5 — khám nick, hỏi người dùng, lập hồ sơ (KHÔNG chạy `node cai.mjs` vì nó hỏi trong terminal)
1. `node kham-nick.mjs --liet-ke-nhan` → in JSON `{uid, ho_so_da_co, nhan:[{ten, so}]}` (mất 1–5 phút, nó đọc bộ lọc nhãn Zalo, không mở hội thoại). Nếu `ho_so_da_co` là true → hỏi người dùng có muốn khám lại không; không thì nhảy tới bước 6.
2. Hỏi người dùng (một lượt, gọn — dùng AskUserQuestion nếu có, không thì hỏi trong chat):
   ...
   - Từ danh sách nhãn vừa in: nhãn nào là **khách hàng** (bot được trả lời), **bạn bè/đối tác** (bot trả lời và tự nhận là AI), **chỉ đọc** (báo họ, không trả lời), **cấm** (gia đình, riêng tư — bot không đọc). Nhãn không nhắc tới = bot không đụng. Không có nhãn nào → gợi ý họ gắn nhãn trong Zalo (Phân loại) rồi làm lại, hoặc tạm chỉ dùng buồng lái.
...
3. Ghi câu trả lời vào `tra-loi.json` trong thư mục gói:
   ...
   { "ten": "…", "ten_goi": "anh …", "xung": "a", "mo_ta": "…", "mat_hang": ["…"], "tu_khoa": ["…"],
     "nhan_tra_loi": ["…"], "nhan_tro_ly": ["…"], "nhan_chi_doc": ["…"], "nhan_cam": ["…"], "buong_lai": "…" }
   ```
   Tên nhãn chép **nguyên văn** từ JSON bước 5.1.
...
## Bước 7 — bật bot ở chế độ NHÁP và bàn giao
- Chạy nền: `node xu-ly-ca.mjs` (mac có `./chay.sh`, win có `chay.cmd`). Bot trực, có tin mới ở nhãn được phép thì nháp xuất hiện trong nhóm buồng lái.
- Nói cho người dùng: cách dừng (Ctrl-C ở cửa sổ đang chạy), cách chạy lại (`./chay.sh` / `chay.cmd`), cách thử gửi thẳng cho một người (`--nick "Tên" --gui`), cách nâng cấp (chạy lại chính câu họ đã dán cho bạn), và rằng mọi việc tiếp theo (thêm nhãn, đổi cách xưng, dạy giọng, nối kho) họ chỉ cần nói với bạn — bạn đọc `SKILL.md` mục 5 để làm.
